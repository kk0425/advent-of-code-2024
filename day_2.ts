...
function checkReportDampener(report: Array<number>): boolean {
  if (checkReport(report)) {
    return true;
  }
  for (let i = 0; i < report.length; i++) {
    const modifiedReport = report.filter((_value, index) => index !== i);
...
function part2(input: string): number {
  const reports = parse(input);
  const safeReports = reports.filter(checkReportDampener);
  return safeReports.length;
}
